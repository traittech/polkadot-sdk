	...
	Client::Api: Metadata<Block>,
{
	...
	fn storage_diff_with_prefixes(
		...
		// TODO 1. it may be possible that iters yield not only the leafs, but also interim nodes
		// i.e. collection keys. If so - need to skip them.

		// TODO 2. Need to implement compare operations for StorageKey: ==, <, >
		// EXACTLY the same way as it's done for items in sp-trie package
		// See here:
		// https://github.com/paritytech/trie/blob/6eb4b88e0781da2db7dda0021249a8fa4244bc5d/trie-db/src/trie_codec.rs#L229

		// TODO 3. Unlike other functions in this file - we make many requests to the same block state.
		// Need to ensure that the state remains existing & not deleted/pruned till the end of the processing.


		let start_state = self.client.state_at(start)?;
		let end_state = self.client.state_at(end)?;

		let start_iter = start_state.raw_iter(IterArgs::default())?;
		let end_iter = end_state.raw_iter(IterArgs::default())?;

		let mut start_key: StorageKey = start_iter.next_key()?.ok();
		let mut end_key: StorageKey = end_iter.next_key()?.ok();

		let mut storage_diff : Vec<(StorageKey, Option<StorageData>)> = vec![];

		// Due to the structure of the State Trie - leafs of the trie, that contain the storage keys, are sorted.
		// Iterator generated by the Trie provides sorted depth-first view on the leafs.
		// This allows to implement an efficient algorithm - direct comparing of storage keys yielded by two iterators.
		loop {
			// skip keys
			if !is_target_key(start_key) {
				// scroll to the next key
				start_key = start_block_iter.next_key()?.ok();

				continue;
			}
			if !is_target_key(end_key) {
				continue;

				// scroll to the next key
				end_key = end_iter.next_key()?.ok();
			}

			// compare keys
			if start_key == end_key {
				// keys are equal, check hashes of the storage items

				let start_hash = start_state.storage_hash(start_key).map_err(client_err);
				let end_hash = end_state.storage_hash(end_key).map_err(client_err);
				
				if start_hash != end_hash {
					// storage item was updated
					let end_storage_value = end_state.storage(end_key).map_err(client_err)?;
					storage_diff.push((end_key.clone(), end_storage_value));
				}

				// scroll to the next keys
				start_key = start_block_iter.next_key()?.ok();
				end_key = end_iter.next_key()?.ok();

			} else if start_key < end_key {
				// the storage item was deleted
				storage_diff.push((start_key.clone(), None));

				// scroll to the next key
				start_key = start_block_iter.next_key()?.ok();
			} else {
				// the storage item was added
				let end_storage_value = end_block_state.storage(end_key).map_err(client_err)?;
				storage_diff.push((end_key.clone(), end_storage_value));
